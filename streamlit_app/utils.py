...
def _theme_variable_block(mode):
    if mode == "dark":
        return """
<style>
:root {
...
def render_theme_controls():
    ...
    if mode in {"dark", "light"}:
        st.markdown(_theme_variable_block(mode), unsafe_allow_html=True)
...
def get_company_options():
    ...
    for ticker, company in sorted(
        COMPANY_MAPPING.items(),
        key=lambda x: x[1]
    ):

        display = f"{company} ({ticker})"

        options.append(display)

        reverse_mapping[display] = ticker

    return options, reverse_mapping